...
// Endpoint: https://msrnext.netlify.app/api/shiprocket-sync
...
export default async (req) => {
  // 1. Handle CORS Preflight & Health Checks (Shiprocket validation pings with GET / OPTIONS / HEAD)
  if (req.method === 'GET' || req.method === 'HEAD' || req.method === 'OPTIONS') {
    return new Response(
      JSON.stringify({
        status: 'active',
        service: 'MSR Next Gen Shiprocket Webhook Listener',
  ...
  // 2. Handle POST Tracking Webhook from Shiprocket
  if (req.method === 'POST') {
    try {
      ...
      return new Response(JSON.stringify({ error: err.message }), {
        status: 200, // Return 200 so webhook validator succeeds
